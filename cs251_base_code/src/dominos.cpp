...
namespace cs251
{
    ...
    */
    ...
    nMet = 2; nMag = 2;
    mspherebody = new b2Body* [nMet];
    spherebody = new b2Body* [nMag];
    ...
    {
        mballbd.position.Set(-5.0*i+i*8, 34);
        mspherebody[i] = m_world->CreateBody(&mballbd);
        mspherebody[i]->CreateFixture(&mballfd);
    }
    //A magnetic ball
    ...
    {
        ballbd.position.Set(10+i*20, 26.6f);
    ...
    void dominos_t::applyForces()
    {
        ...
        for(int j=0;j<nMet;j++)
        {
            for(int i=0;i<nMag;i++)
            {
                b2Vec2 c1 = spherebody[i]->GetWorldCenter();
                b2Vec2 c2 = mspherebody[j]->GetWorldCenter();
            ...
            mspherebody[j]->ApplyForceToCenter( force,true);
        }
    }
